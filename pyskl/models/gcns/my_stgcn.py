...
class mySTGCN(nn.Module):
    ...
    def __init__(self,
                 ...
                 inflate_stages=[5, 8],
                 down_stages=[5, 8],
                 frozen_stages= 9,  # default= -1 --> max=9 cause num_stages=10
                 pretrained=None,
                 **kwargs):
        super().__init__()
    ...
    def _freeze_stages(self):
        ...
        # First loop is independent of the frozen_stages value
        # Freezes all stages
        ...
        # This loop depends on the frozen_stages value
        # if frozen_stages=9 its the same with above
        for i in range(1, self.frozen_stages + 1):
            self.gcn[i].eval()
            for param in self.gcn[i].parameters():
                 param.requires_grad = False
    ...
    def train(self, mode=True):
        ...
        train func --> overrides method in Module
        """
        super().train(mode)
        self._freeze_stages()
